...
<#   
  ...
  PS Z:\GMO\_PS\QY>   ConvertCSV-ToExcel -inputfile @("Files-ConvertCVS-ToExcel\test1.csv", "Files-ConvertCVS-ToExcel\test2.csv") -output 'Files-ConvertCVS-ToExcel\report.xlsx'  

  .EXAMPLE      
  ConvertCSV-ToExcel -inputfile @("test1.csv","test2.csv") -output 'report.xlsx'
  PS Z:\GMO\_PS\QY>   ConvertCSV-ToExcel -inputfile 'Files-ConvertCVS-ToExcel\test1.csv' -output 'Files-ConvertCVS-ToExcel\report.xlsx'
  .NOTES  
...
End {
    #Save spreadsheet
    #$workbook.saveas("$pwd\$output")        # QY: comment out, calling ps1 from xp_cmdshell
    #$thisPath = "D:\GMOPowershell\Files-ConvertCVS-ToExcel\$((get-date).toString('MMddyyyy'))"
    $thisPath = $serverName.Replace("\" , "-") + "_" + $((get-date).toString('MMddyyyy'))
    $thisPath = "D:\GMOPowershell\Files-ConvertCVS-ToExcel\$thisPath"
     
    $workbook.saveas("$thisPath\$output") 

    #Write-Host -Fore Green "File saved to $pwd\$output"
    Write-Host -Fore Green "File saved to $thisPath\$output"
