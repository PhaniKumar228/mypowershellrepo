...
$query = "SET NOCOUNT ON; "
        $query = $query + "truncate table DRDMirroringStatus "
        $query = $query + "exec spDRD_GetServerList 'Mirroring'"
 ...
 foreach($a in $dataSet.Tables[0].Rows)
{
  write-host "DBServer: " $a.InstanceName; 
   ...
   # open a connection for the server/database
    ...
    $tableName = "DRDMirroringStatus"
    
    $sql = gc "$dir\collectmirroringstatus.sql" | out-string #out-string is important 
   # write-host $sql 
