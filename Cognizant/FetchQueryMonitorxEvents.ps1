...
$query = "SET NOCOUNT ON;"
 ...
 #$query = $query + " @ServerList = 'MARSqlPrd71B\INST2' "
        if ($ServerList -ne "" )
        {
 ...
 foreach($a in $dataSet.Tables[0].Rows)
{
  ...
  $start=Get-Date;
   ...
   # open a connection for the server/database
     ...
     try
    {
		...
		elseif ($a.Servername -eq "BOSSQLUAT123A")
		{
			$EVTFileLocation = "G:\Ops\ExtEvents\";
		}
		elseif ($a.Servername -eq "MARSQLPRD34")
		{
			$EVTFileLocation = "G:\Ops\ExtEvents\";
		}
		else
 ...
 #   $sql =  $sql + " where convert(datetime,substring(event_data,charindex('timestamp=',event_data)+11,24)) > '" +$a.xEventTimeStamp + "'; "
  ...
  $sql =  $sql + " event_data_XML.value ('(/event/data  [@name=''duration''      ]/value)[1]', 'bigint'        )/1000 AS duration_ms, "
  $sql =  $sql + " event_data_XML.value ('(/event/data  [@name=''object_type''   ]/text)[1]', 'varchar(100)'        ) AS object_type, "
  $sql =  $sql + " DB_Name(event_data_XML.value ('(/event/action  [@name=''database_id''   ]/value)[1]', 'bigint'        )) AS DatabaseName, "
  ...
  $sql =  $sql + " event_data_XML.value ('(/event/data  [@name=''cpu_time''      ]/value)[1]', 'BIGINT'        )/1000 AS cpu_time_ms, "
  ...
  $sql =  $sql + "   CASE event_data_XML.value ('(/event/@name)[1]', 'vARCHAR(50)'     ) "
  $sql =  $sql + " 	when 'sql_batch_completed' then  event_data_XML.value ('(/event/data  [@name=''batch_text''     ]/value)[1]', 'NVARCHAR(max)')  "
  $sql =  $sql + " 	ELSE event_data_XML.value ('(/event/data  [@name=''statement''     ]/value)[1]', 'NVARCHAR(max)')  "
...
# write-host $sql
    ...
    try{
        ...
        $commandSourceData.CommandTimeout = '1800'
    ...
    Catch
    {
        ...
        {
				write-host "Offset issue relating to evt file rollover, attempting to fetch all";
			    ...
			    $sql =  $sql + " INTO #Events "
  				$sql =  $sql + " FROM sys.fn_xe_file_target_read_file('" +$EVTFileLocation +"sql2012*.xel', null,null,null) AS F "
			...
			{
				Write-Host "Write-DataTable$($connectionName):$ex1.Message";
			}
    }
